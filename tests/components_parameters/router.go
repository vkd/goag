...
func (rt *API) routeShopsShopString(path, method string) (http.Handler, string, bool) {
	prefix, path := splitPath(path)
...
func (rt *API) routeShopsShopStringSep(path, method string) (http.Handler, string, bool) {
	_, path = splitPath(path)

	return rt.routeShopsShopStringSepShopSchema(path, method)
}

func (rt *API) routeShopsShopStringSepShopSchema(path, method string) (http.Handler, string, bool) {
	prefix, path := splitPath(path)
	...
	if path == "" {
		...
		case "/pets":
			...
			case http.MethodPost:
				h := http.Handler(rt.PostShopsShopStringSepShopSchemaPetsHandler)
				return h, "/shops/{shop_string}/sep/{shop_schema}/pets", true
			}
		}
		return nil, "", false
	}
