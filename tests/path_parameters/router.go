...
func (rt *API) routeShopsShopString(path, method string) (http.Handler, string, bool) {
	...
	return rt.routeShopsShopStringShopSchema(path, method)
}

func (rt *API) routeShopsShopStringShopSchema(path, method string) (http.Handler, string, bool) {
	prefix, path := splitPath(path)
	...
	if path == "" {
		...
		case "/pets":
			...
			case http.MethodPost:
				h := http.Handler(rt.PostShopsShopStringShopSchemaPetsHandler)
				return h, "/shops/{shop_string}/{shop_schema}/pets", true
			}
		}
		return nil, "", false
	}
